...
no real network, no real config/API files, no real filesystem outside
`tmp_path` (cwd is chdir'd to a fake `src/` so the EMA/feedback routes'
hardcoded `"../logs/..."` relative paths resolve safely).
"""
...
def test_access_ema_participant_not_found(routes_client, routes_app_instance):
    routes_app_instance.participant_manager.get_participant.return_value = None
    resp = routes_client.get('/EMA/access_ema/1')
    assert resp.status_code == 404


def test_access_ema_schema_mismatch_crashes(routes_client, routes_app_instance):
    """Documents a known, deliberately-deferred bug (see plan/02-server-pytest.md):
    this route still reads participant['first_name']/['last_name'], but the
    real participant schema (_participant_manager.py) was migrated to
    initials/subid. Against real (post-migration) participant data, this
    raises an uncaught KeyError (Flask's test client propagates it rather
    than converting it to a 500 response, unlike a real WSGI server) --
    pinned here, not fixed, since the fix touches an external HTTP contract
    (Qualtrics) with unconfirmed callers.
    """
    routes_app_instance.participant_manager.get_participant.return_value = {
        'unique_id': '1', 'initials': 'JD', 'subid': '3000',
    }
    with pytest.raises(KeyError):
        routes_client.get('/EMA/access_ema/1')


def test_access_ema_first_time_today(routes_client, routes_app_instance):
    routes_app_instance.participant_manager.get_participant.return_value = {
        'unique_id': '1', 'first_name': 'Jane', 'last_name': 'Doe',
    }
    resp = routes_client.get('/EMA/access_ema/1')
    assert resp.status_code == 200
    body = resp.get_json()
    assert body['subject_name'] == 'Jane Doe'
    assert 'first time' not in body['status'].lower() or 'time to take' in body['status'].lower()


def test_request_coords_found(routes_client, routes_app_instance):
    routes_app_instance.participant_manager.get_coords.return_value = [{'latitude': 1.0, 'longitude': 2.0}]
    resp = routes_client.get('/EMA/request_coords/1')
    assert resp.status_code == 200


def test_request_coords_not_found(routes_client, routes_app_instance):
    routes_app_instance.participant_manager.get_coords.return_value = None
    resp = routes_client.get('/EMA/request_coords/1')
    assert resp.status_code == 404


def test_submit_ema_success(routes_client):
    resp = routes_client.post('/EMA/submit_ema', json={'participantID': '1', 'subjectName': 'Jane Doe'})
    assert resp.status_code == 200


def test_submit_ema_missing_fields(routes_client):
    resp = routes_client.post('/EMA/submit_ema', json={'participantID': '1'})
    assert resp.status_code == 400


def test_submit_ema_non_dict_body_is_clean_400(routes_client):
    resp = routes_client.post('/EMA/submit_ema', data='42', content_type='application/json')
    assert resp.status_code == 400
...
def test_access_feedback_participant_not_found(routes_client, routes_app_instance):
    routes_app_instance.participant_manager.get_participant.return_value = None
    resp = routes_client.get('/feedback_survey/access_feedback/1')
    assert resp.status_code == 404


def test_access_feedback_schema_mismatch_crashes(routes_client, routes_app_instance):
    """Same known/deferred first_name/last_name schema mismatch as
    test_access_ema_schema_mismatch_crashes above, in the feedback route.
    """
    routes_app_instance.participant_manager.get_participant.return_value = {
        'unique_id': '1', 'initials': 'JD', 'subid': '3000',
    }
    with pytest.raises(KeyError):
        routes_client.get('/feedback_survey/access_feedback/1')


def test_access_feedback_success(routes_client, routes_app_instance):
    routes_app_instance.participant_manager.get_participant.return_value = {
        'unique_id': '1', 'first_name': 'Jane', 'last_name': 'Doe',
    }
    routes_app_instance.participant_manager.get_lapse_data_and_message.return_value = {
        'lapse_level': 'high', 'lapse_change': 'increasing',
        'most_important_feature': 'craving', 'message': 'sample',
    }
    resp = routes_client.get('/feedback_survey/access_feedback/1')
    assert resp.status_code == 200
    assert resp.get_json()['subject_name'] == 'Jane Doe'


def test_submit_recommendations_success(routes_client):
    resp = routes_client.post('/feedback_survey/submit_feedback', json={'participantID': '1', 'subjectName': 'Jane Doe'})
    assert resp.status_code == 200


def test_submit_recommendations_missing_fields(routes_client):
    resp = routes_client.post('/feedback_survey/submit_feedback', json={'participantID': '1'})
    assert resp.status_code == 400


def test_submit_recommendations_non_dict_body_is_clean_400(routes_client):
    resp = routes_client.post('/feedback_survey/submit_feedback', data='42', content_type='application/json')
    assert resp.status_code == 400
